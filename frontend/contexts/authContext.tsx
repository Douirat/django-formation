...
const authenticate = async (): Promise<User | null> => {
  ...
  try {
     const res = await fetch("http://127.0.0.1:8000/authentication/authenticate/", {
    ...
    return (
        <authContext.Provider value={{chosen_user, chooseCostumer, chooseCompany, loggedUser, toggleToCostumer, toggleToCompany, setTheLoggedUser, authenticate, Logout }}>{children}</authContext.Provider>
    )
}
